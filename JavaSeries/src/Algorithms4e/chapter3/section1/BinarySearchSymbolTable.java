...
import Algorithms4e.chapter1.section3.Queue;
 ...
 * see {@link SymbolTable}, {@link BinarySearchSymbolTable}, {@link SequentialSearchSymbolTable}
...
public class BinarySearchSymbolTable<Key extends Comparable<Key>, Value> implements OrderedSymbolTable<Key, Value> {
    ...
    public Key ceiling(Key key) {
        if (key == null) throw new IllegalArgumentException("argument to floor() is null");
        int i = rank(key);
        if (i == n) throw new NoSuchElementException("argument to ceiling() is to large");
        else return keys[i];
    }
